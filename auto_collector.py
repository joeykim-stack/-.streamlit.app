...
# 💡 엑셀 데이터는 1시간(3600초) 캐싱 (로딩 속도 대폭 향상)
...
# 💡 API 통신은 10분(600초) 캐싱! (한 번만 30초 기다리면 그 뒤론 광속)
...
def fetch_api_data_raw():
    ...
    try:
        ...
        return df_api_clean, f"🟢 실시간 데이터 수집 성공! (신규 {len(df_api_clean)}건 추출)"
    except Exception as e:
...
def get_processed_data_raw():
    ...
    with st.spinner('⏳ 조달청 실시간 데이터를 스캔 중입니다... (최초 1회 약 30초 소요)'):
...
# --- 7. UI ---
st.markdown(f"<div class='main-title'>🏆 조달청 통합 대시보드 v72.5 (Simple & Classic)</div>", unsafe_allow_html=True)
