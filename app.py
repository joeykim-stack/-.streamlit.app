...
# 1. 페이지 설정 및 디자인
st.set_page_config(page_title="Procurement Dashboard v5.1", layout="wide")
...
st.markdown("""
    <div class="sticky-header">
        <h1 style="margin: 0; padding: 0;">🏆 통합 조달 전략 분석 대시보드 v5.1</h1>
    </div>
""", unsafe_allow_html=True)
...
if not df_raw.empty:
    ...
    if df_f.empty:
        ...
        # 🚨 [수정 포인트 1] 직관적인 2D 콤보 트렌드 차트 (막대 + 꺾은선 이중 축)
        ...
        # 매출액 바 차트
        fig_trend.add_trace(
            go.Bar(x=trend['월'], y=trend['금액'], name="매출액", marker_color='#1e3a8a', opacity=0.85),
            secondary_y=False,
        )
        # 계약건수 라인 차트
        fig_trend.add_trace(
            go.Scatter(x=trend['월'], y=trend['건수'], name="계약건수", mode='lines+markers+text', 
                       text=trend['건수'], textposition="top center", 
                       textfont=dict(color='#ff7f0e', size=13, family="Arial Black"),
                       line=dict(color='#ff7f0e', width=3), marker=dict(size=8, color='#ff7f0e')),
            secondary_y=True,
        )
        ...
        # 🚨 [수정 포인트 2] No. 컬럼 넓이를 픽셀(px) 단위로 강제 초소형 압축
        st.dataframe(
            styled_df, 
            hide_index=True, 
            column_config={
                "No.": st.column_config.NumberColumn("No.", width=40), # 글자만 들어갈 40픽셀로 제한
