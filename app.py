...
# 1. 페이지 설정 및 디자인
st.set_page_config(page_title="Procurement Dashboard v5.2", layout="wide")
...
# --- 2. 데이터 수집 함수 (4/15 이후 실시간 데이터만) ---
def fetch_api_data():
    today = datetime.now().strftime("%Y%m%d")
    # 🚨 API는 4월 15일부터만 호출 (서버 부하 최소화)
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if raw_items:
                ...
                df_api['월'] = "4월"
                df_api['건수'] = 1
                return df_api[df_api['업체명'].str.contains('|'.join(TARGET_COMPANIES), na=False)], "성공 (4/15~)"
    except: pass
    return pd.DataFrame(), "실시간 데이터 없음/대기"

@st.cache_data(ttl=600)
def load_data():
    all_dfs = []
    # 🚨 4월 1일~14일 치 파일 (data04.csv) 추가
...
st.markdown("""
    <div class="sticky-header">
        <h1 style="margin: 0; padding: 0;">🏆 통합 조달 전략 분석 대시보드 v5.2</h1>
    </div>
""", unsafe_allow_html=True)
...
if not df_raw.empty:
    ...
    st.sidebar.write("---")
    st.sidebar.info("💡 1월~4/14: 파일 로드\n\n💡 4/15 이후: 실시간 API")
    if "성공" in api_status: st.sidebar.success(f"✅ {api_status}")
    ...
    if df_f.empty:
        ...
        st.caption("💡 막대는 매출액(좌측 축), 꺾은선은 계약 건수(우측 축)를 나타냅니다.")
        ...
        with col_l:
            ...
            if not comp_data.empty:
                pull_vals = [0.12 if i == 0 else 0 for i in range(len(comp_data))]
        ...
        with col_r:
            ...
            if not cat_data.empty:
                pull_vals2 = [0.12 if i == 0 else 0 for i in range(len(cat_data))]
