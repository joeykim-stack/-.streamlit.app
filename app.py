...
# 1. 페이지 설정 및 디자인
st.set_page_config(page_title="Procurement Dashboard v5.3", layout="wide")
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if raw_items:
                ...
                df_api['월'] = "4월"
                df_api['건수'] = 1
                return df_api[df_api['업체명'].str.contains('|'.join(TARGET_COMPANIES), na=False)], f"연동 성공 (신규 {len(df_api)}건)"
            else:
                # 🚨 핵심: 연결은 됐으나 조달청 서버에 데이터가 아직 없는 경우
                return pd.DataFrame(), "연결 성공: 4/15 데이터 시스템 등록 대기 중"
        elif res.status_code == 429:
            return pd.DataFrame(), "API 한도 초과 (내일 다시 시도)"
        elif res.status_code == 500:
            return pd.DataFrame(), "조달청 서버 점검 중 (500 에러)"
    except Exception as e:
        return pd.DataFrame(), f"네트워크 오류: {str(e)[:20]}"
...
st.markdown(f"""
    <div class="sticky-header">
        <h1 style="margin: 0; padding: 0;">🏆 통합 조달 전략 분석 대시보드 v5.3</h1>
    </div>
""", unsafe_allow_html=True)
...
if not df_raw.empty:
    ...
    # 🚨 API 상태 실시간 보고
    st.sidebar.write("---")
    st.sidebar.markdown("**📡 API 연동 상태**")
    if "연결 성공" in api_status: 
        st.sidebar.info(f"🟢 {api_status}")
        st.sidebar.caption("💡 조달청 시스템은 실적 정산에 보통 1일이 소요됩니다.")
    elif "연동 성공" in api_status:
    ...
    if df_f.empty:
        ...
        fig_trend.update_layout(margin=dict(l=0, r=0, b=0, t=30), height=400, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), plot_bgcolor='rgba(0,0,0,0)')
        fig_trend.update_yaxes(title_text="매출액 (원)", secondary_y=False, showgrid=True, gridcolor='#e9ecef')
        fig_trend.update_yaxes(title_text="계약 건수 (건)", secondary_y=True, showgrid=False)
        ...
        with col_l:
            ...
            if not comp_data.empty:
                fig_pie1 = go.Figure(data=[go.Pie(labels=comp_data.index, values=comp_data.values, hole=0.45, pull=[0.12 if i==0 else 0 for i in range(len(comp_data))], textinfo='percent+label', marker=dict(line=dict(color='#ffffff', width=2), colors=px.colors.sequential.Agsunset))])
                fig_pie1.update_layout(margin=dict(t=20, b=20, l=20, r=20), showlegend=False, title=f"Top 10 업체 점유율 ({selected_period})")
                st.plotly_chart(fig_pie1, use_container_width=True)
        with col_r:
            cat_data = df_pie.groupby('물품분류명')['금액'].sum().sort_values(ascending=False)
            if not cat_data.empty:
                fig_pie2 = go.Figure(data=[go.Pie(labels=cat_data.index, values=cat_data.values, hole=0.45, pull=[0.12 if i==0 else 0 for i in range(len(cat_data))], textinfo='percent+label', marker=dict(line=dict(color='#ffffff', width=2), colors=px.colors.sequential.Tealgrn))])
                fig_pie2.update_layout(margin=dict(t=20, b=20, l=20, r=20), showlegend=False, title=f"품목별 매출 비중 ({selected_period})")
    ...
    st.error("데이터 로드 실패. 깃허브에 파일이 있는지 확인해 주세요.")
