...
# 1. 페이지 설정
st.set_page_config(page_title="조달 분석 솔루션 v3.2", layout="wide")
...
def load_data():
    ...
    for month, path in file_map.items():
        if os.path.exists(path):
            try:
                tmp = pd.read_csv(path, sep='\t', encoding='utf-16', on_bad_lines='skip')
                ...
                c_amt = next((c for c in tmp.columns if '납품금액' in c and '합계' not in c), None)
                if all([c_corp, c_item, c_method, c_amt]):
                    df_sub = tmp.copy()
                    df_sub['업체명'] = df_sub[c_corp].astype(str).str.strip()
                    df_sub['물품분류명'] = df_sub[c_item].astype(str).str.strip()
                    df_sub['계약유형'] = df_sub[c_method].astype(str).str.strip()
                    df_sub['금액'] = pd.to_numeric(df_sub[c_amt].astype(str).str.replace(',', ''), errors='coerce').fillna(0)
                    df_sub['월'] = month
                    df_sub['건수'] = 1
                    df_sub = df_sub[df_sub['업체명'].str.contains('|'.join(TARGET_COMPANIES), na=False)]
                    all_dfs.append(df_sub[['업체명', '물품분류명', '계약유형', '금액', '건수', '월']])
            except: continue
...
if not df_raw.empty:
    ...
    st.title("📊 조달 실적 통합 정밀 분석 대시보드 v3.2")
    ...
    res_df['1분기 합계(액)'] = pivot_amt[q1_months].sum(axis=1)
    res_df['1분기 합계(건)'] = pivot_cnt[q1_months].sum(axis=1)
